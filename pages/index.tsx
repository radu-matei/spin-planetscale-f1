...
const fermyonUrl = "/data.json";

const Home: NextPage = () => {
	const { resolvedTheme } = useTheme();
	const [edgeFunctionUrl, _setEdgeFunctionUrl] = useState(fermyonUrl);
	const [lineChartMemoizeKey, _setLineChartMemoizeKey] = useState(0);
	...
	useEffect(() => {
		if (data) {
			...
			setRaceDates(
				data?.races.map((item) => {
					return f
						.formatToParts(Date.parse(item.date))
						.reduce((acc, part) => ({ ...acc, [part.type]: part.value }), {});
				}) ?? [],
			);
			setRaceNames(data.races.map((item) => circuitName(item.race_name)));

			const teams: Record<string, Constructor> = data.results.reduce(
				(acc, item) => {
					if (!acc[item.teamId]) {
	...
	return (
		<>
			<Head>
				<title>
					F1 Constructors Championship standings using Fermyon Cloud and
					PlanetScale!
				</title>
				<meta
					name='description'
					content='F1 Constructors Championship standings week-by-week. Powered by Fermyon Cloud and PlanetScale.'
				/>
			...
			<header className='container mx-auto items-end justify-between space-y-2 px-3 pt-4 pb-4 md:flex md:px-6 md:pb-2 md:pt-8'>
				<h1 className='text-xl font-semibold lg:text-2xl'>
					...
					Constructor championship standings built with Fermyon Cloud and
					PlanetScale!
				</h1>
			</header>
								...
								>
									<div
										className={`flex space-x-xs lg:block lg:translate-y-xs ${
											i === selectedRound
												? "lg:text-gray-50 lg:dark:text-gray-900"
												: ""
										}`}
									>
										<div className='text-3xs font-bold leading-none tracking-tighter'>
											{date.month}
										</div>
										<div className='text-3xs font-bold leading-none lg:text-sm lg:font-normal'>
											{date.day}
										</div>
									</div>
								</button>
										...
										<div
											...
											className={`border-b border-r border-r-gray-50 [border-bottom-style:dashed] dark:border-r-gray-850 ${
												i % 21 === selectedRound - 1
													? "border-r-gray-800 dark:border-r-gray-100"
													: ""
											}`}
										/>
									))}
					...
					<section className='container mx-auto px-3 pt-2 pb-6 md:px-6'>
						<div className='space-y-1 border-y py-3'>
							...
							<p className='lg:w-3/4'>
								This is powered by Fermyon Cloud and a PlanetScale database.
								Fermyon Cloud uses{" "}
								<a
									href='https://github.com/planetscale/database-js'
									className='text-blue'
								>
									@planetscale/database
								</a>{" "}
								to query data over HTTP. Adapted from
